...
class ConfigHookPlugin:
    ...
    def on_config_change(
        self, event: str, old_config: Dict[str, Any], new_config: Dict[str, Any]
    ):
        pass
    ...
    def fetch(self) -> Dict[str, Any]:
        ...
        for attempt in range(1, self.retries + 1):
            try:
                ...
                time.sleep(1)
        logger.error(
            f"HTTPConfigLoader failed after {self.retries} attempts: {last_exc}"
        )
        raise last_exc
...
class ConfigManager:
    ...
    def __init__(
        ...
        # Encryption support
        self.encryption = (
            ConfigEncryption(encryption_key) if enable_encryption else None
        )
        ...
        with self._lock:
            ...
            try:
                ...
                if (
                    ...
                    and self.encryption.is_encrypted(current)
                ):
                    current = self.encryption.decrypt(
                        current[4:]
                    )  # Remove 'enc:' prefix
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy",
            "config_files_loaded": len(
                [f for f in self.config_files if os.path.exists(f)]
            ),
